 ...
 * #############################################################################
 *
 * Configuration for the mechanical endstops
 */

/**
 * By default all endstops are pulled up to HIGH. You need a pull-up if you use
 * a mechanical endstop connected with GND. Set value to false for no pull-up on
 * this endstop.
 */
 ...
 * connected properly. If your axes move in one direction ONLY when the
 * endstops are triggered, set ENDSTOPS_INVERTING to true here
 */
...
/**
 * If you have a mirrored motor you can put a second endstop to that motor. On
 * homing you would then need to trigge rboth endstops. Each endstop only stopps
 * one motor, so they are aligned after homing. After homing only the first
 * endstop gets used.
 */
...
/**
 * Which type of endstop when homing is executed (min/max?)
 *
